...
def res(jobfile):
	...
	def semanticSearch(searchString, searchSentencesList):
		...
		scores_skills = {}
		best_skills = {}
		for resume in searchSentencesList: 
			res = resume.split(" ")
			matches = 0
			skills = 0
			bestc = 0
			...
			for x in query:
				if x in res:
					...
					if(val > bestc):
						bestc = val
						bestval = x
			best_skills[Resumes_File_Names[ind]] = bestval
			scores_matches[Resumes_File_Names[ind]] = matches
			scores_skills[Resumes_File_Names[ind]] = skills
			ind += 1
		for x in scores_matches:
			scores_matches[x] = 0.7 * scores_skills[x] + 0.3 * scores_matches[x]
		sorted_x = sorted(scores_matches.items(), key=lambda kv: kv[1], reverse=True)
		return (sorted_x, best_skills, scores_skills)
	...
	print("\n")
	sorted_x, best, scores_skills = semanticSearch(jobfile, Resumes)
	print("\n")
	...
	for x in sorted_x:
		Z.append(str(x[0]) + "      | Skills Matched: " + str(scores_skills[x[0]]) +     "        | Best Skill: " + str(best[x[0]]))
	# print("ZZZZZZZ", Z)
	flask_return = []
